...
| `sdks/` | The bridged provider SDKs (`b2`, `unifi`, `zerotier`), generated by `pulumi install` from the `packages:` block of `Pulumi.yaml` and committed; a test holds each to the block, and renovate moves the pins (docs/framework/ci.md §3). |
...
mint` writes. The appliance is provisioned by `state-backend`, not by a stack:
that command builds the backend every stack's configuration is reached through,
so its own key cannot wait in one (docs/credentials.md §3). Every other
provider credential is a secret in the committed configuration of the
stack that reads it, which the passphrases above open. The
