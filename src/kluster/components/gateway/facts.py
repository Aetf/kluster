"""Reading site facts out of stack configuration.

Most of what the gateway needs is a decision, and decisions are code. A few
things are not: an image digest is whatever the build produced, a node
identifier was minted by the device it belongs to, and the resolvers' addresses
were written into every lease on the LAN long before this program existed. Those
...
def mapping(value: object, what: str) -> dict[str, object]:
    ...
    read an unquoted name as something else, and a member called `true` is a
