...
Records are plain data (`model`, `zones`, `legacy`), the split-horizon
rewrites are derived from the shared route census (`routes`, over
`conventions.ROUTES`), and the two things that turn data into resources are
`zone.ManagedZone` and `adguard.declare_rewrites`, the latter over the custom
provider in `kluster.providers.adguard_rewrites`.
"""
