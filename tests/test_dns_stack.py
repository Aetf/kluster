...
class Mocks(pulumi.runtime.Mocks):
    def new_resource(self, args: pulumi.runtime.MockResourceArgs) -> tuple[str | None, dict[str, Any]]:
        ...
        if args.typ == 'pulumi:pulumi:StackReference':
            outputs['outputs'] = {'cluster_endpoint': LB_ADDRESS, 'vip1': VIP1_ADDRESS}
...
def test_the_anchors_live_only_in_the_primary_zone() -> None:
    ...
    # are CNAMEs to the primary's anchor.
    for zone in conventions.ALL_ZONES:
        if zone == conventions.ZONE_PRIMARY:
            continue
        assert conventions.ANCHOR_CLUSTER not in {record['name'] for record in _records_of(zone).values()}, zone


def test_the_vip_anchor_is_declared() -> None:
    anchor = _records_of(conventions.ZONE_PRIMARY)[f'{conventions.ZONE_PRIMARY}-{conventions.ANCHOR_VIP1}-a']
