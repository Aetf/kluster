...
def test_keys_are_deterministic() -> None:
    assert pki.server_credential(SEED, ADDRESS).key_pem == pki.server_credential(SEED, ADDRESS).key_pem


def test_roots_separate_keys() -> None:
    other = bytes(range(1, 33))
    assert pki.server_credential(SEED, ADDRESS).key_pem != pki.server_credential(other, ADDRESS).key_pem


def test_clients_have_distinct_keys() -> None:
    assert pki.client_credential(SEED, 'ci').key_pem != pki.client_credential(SEED, 'operator').key_pem


def test_server_certificate_carries_the_ip() -> None:
    cert = x509.load_pem_x509_certificate(pki.server_credential(SEED, ADDRESS).cert_pem)
...
def test_client_common_name_is_the_role() -> None:
    cert = x509.load_pem_x509_certificate(pki.client_credential(SEED, 'ci').cert_pem)
    assert cert.subject.rfc4514_string() == 'CN=ci'


def test_unknown_client_is_rejected() -> None:
    with pytest.raises(ValueError):
        _ = pki.client_credential(SEED, 'root')


def test_leaves_are_issued_by_the_ca() -> None:
    ca = x509.load_pem_x509_certificate(pki.ca_credential(SEED).cert_pem)
    for credential in (pki.server_credential(SEED, ADDRESS), pki.client_credential(SEED, 'operator')):
...
def test_ca_is_a_ca_and_leaves_are_not() -> None:
    ca = x509.load_pem_x509_certificate(pki.ca_credential(SEED).cert_pem)
    leaf = x509.load_pem_x509_certificate(pki.client_credential(SEED, 'ci').cert_pem)
...
def test_openssl_accepts_the_chain(tmp_path: Path) -> None:
    ...
    _ = ca_file.write_bytes(pki.ca_credential(SEED).cert_pem)
    _ = server_file.write_bytes(pki.server_credential(SEED, ADDRESS).cert_pem)
    proc = sp.run(
