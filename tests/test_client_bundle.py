...
def test_the_url_names_files_that_exist(tmp_path: Path) -> None:
    bundle = config.client_bundle(SEED, name='operator', address='192.0.2.10')
...
def test_the_url_pins_the_server_by_address(tmp_path: Path) -> None:
    ...
    config.write_client_bundle(config.client_bundle(SEED, name='ci', address='192.0.2.10'), tmp_path)
