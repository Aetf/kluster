...
The one thing that has to be shared is a kit that is not a file. A real
KeePass database is the right thing to test the row shape against (§2), and
the wrong thing to run a fault sweep against: every open and every write
re-derives the key, which is the whole cost of a suite that runs one
...
@pytest.fixture
def kits() -> Callable[[], KdbxStore]:
    """Hands out fresh in-memory kits, for tests that need more than one."""
    return MemoryKit
