...
| nspawn rootfs (caddy, AdGuard, ZeroTier) | renovate here — docker datasource, reading whole references off the `versions:image-gateway-…` pins | gw-config provider push (pull from the registry, verify, flatten, unpack) | Reviewed; the run-number tag reads as a major bump, so every one waits on dashboard approval |
