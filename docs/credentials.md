...
| Drill age identity | to be generated beside the backup identities and escrowed nowhere; unminted today, `provision` generates the escrowed labels alone (state-backend.md §5) | Decrypts the *latest* dump alone; retention coverage stays with the escrowed generations | ops-repo Environment (`drill`, the private half; pending) · on-box (public half, the third Butane recipient; pending) | Quarterly rebuild drill (state-backend.md §7.3) |
