...
| --- | --- | --- | --- | --- |
| OCI API key (`physical`) | OCI seed key | Its own user, group and policy; administrator of the `physical` compartment and a stranger outside it | Pulumi config secret · CI env (pending) | `physical` |
...
| Cloudflare token (zones) | CF seed token | DNS edit, estate zones only | Pulumi config secret · CI env (pending) | `dns`, `apps` |
...
| B2 management key | B2 seed key | Bucket/key/lifecycle admin, **no file capabilities** | Pulumi config secret · CI env (pending) | `physical` |
...
| Drill age identity | generated beside the backup identities and escrowed nowhere | Decrypts the *latest* dump alone; retention coverage stays with the escrowed generations | ops-repo Environment (`drill`, the private half; pending) · on-box (public half, the third Butane recipient) | Quarterly rebuild drill (state-backend.md §7.3) |
...
| UDM SSH key, libvirt SSH identity | installed by the estate's other automation: gw-config puts the UDM key on the gateway, aconfmgr provisions the homelab host's dedicated service user together with its key (physical/homelab-host.md §4) | gw-config push (host key pinned) / `virsh` as a `libvirt`-group user | Pulumi config secret · CI env (pending) | `physical` |
| UniFi API key | Dedicated local admin | Network API | Pulumi config secret · CI env (pending) | `physical` |
| AdGuard API credentials | AdGuard admin (no scoped API — audit M6) | alice/bob rewrite API | Pulumi config secret · CI env (pending) | `dns` rewrites |
| ZeroTier Central API token | Made in the Central console (no token API) | The whole Central account: the estate's network, its members and its flow rules | Pulumi config secret (`zerotierApiToken`; the network id beside it is a constant in `conventions`, not a secret) · CI env (pending) | `physical` |
