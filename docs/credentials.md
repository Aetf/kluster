...
| GitHub actions-trigger PAT | kluster **Actions: write** only — fires `workflow_dispatch` runs, cannot push code | ops-repo secret | Weekly drift trigger (ci.md §3) | GitHub expiry + reminder e-mail |
...
| Alertmanager read token | `GET /api/v2/alerts` only — read-only by HTTPRoute method+path+header match (cluster-infra.md §1) | ops-repo secret · embedded in the HTTPRoute spec (k8s-base render, Pulumi config secret) | Issue-sync poller | Yearly |
