...
-   **Worker Nodes**: GCP Compute Engine instances handle cloud ingress; Homelab VM handles internal workloads.
...
graph TD  
 ...
 subgraph "GCP VPC (Dual-Stack)"  
 GCP_Worker[GCP Worker Node <br> Public IPv4 & IPv6]  
...
VLAN_IoT(IoT VLAN)  
 ...
 %% Routing Flow  
 User -- "AAAA/A Record" --> GCP_Worker  
 DMSE -- "FRR BGP Peering" --> Homelab_VM  
 DMSE --- VLAN_IoT  
 DMSE --- ZT_Net  
   
%% KubeSpan  
 GCP_Worker <== "KubeSpan (WireGuard UDP 51820)" ==> Homelab_VM  
...
In GCP, L2 announcements and arbitrary BGP IP broadcasts are blocked in the VPC.

-   **Implementation**: Enable Cilium Gateway API with hostNetwork: true. Cilium deploys Envoy directly to the GCP node's host interfaces (ports 80/443).
-   **Load Balancing**: Use standard A/AAAA DNS records pointing directly to the GCP node's public IP. eBPF routes traffic from Envoy to the correct internal pods.
...
-   **Pod Compromise**: mTLS does not prevent a compromised pod from using its legitimate access. Therefore, we use Cilium Network Policies (eBPF L3/L4/L7 rules) to strictly isolate namespaces and pods, which is superior to complex sidecar-based mTLS.
